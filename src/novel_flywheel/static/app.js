...
function renderProviders() {
  ...
  $("#provider-list").innerHTML = state.providers.length ? state.providers.map(p => `<div class="data-row"><div><strong>${escapeHtml(p.name)}</strong><div class="skill-meta">${escapeHtml(p.protocol)} · ${escapeHtml(p.base_url)}</div>${p.models.map(m => `<div class="model-row"><strong>${escapeHtml(m.display_name)}</strong><div class="model-actions"><button class="secondary" data-probe-provider="${p.id}" data-probe-model="${m.id}">检测模型</button><span id="probe-${m.id}" class="probe-result">尚未检测</span></div></div>`).join("")}</div><span class="badge">${p.models.length} 个模型</span></div>`).join("") : '<p class="skill-meta">尚未配置供应商</p>';
