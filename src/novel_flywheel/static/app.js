...
function renderProviders() {
  ...
  document.querySelectorAll("[data-probe-model]").forEach(button => button.addEventListener("click", async () => {
    ...
    try { const result=await api(`/api/providers/${button.dataset.probeProvider}/models/${button.dataset.probeModel}/probe`,{method:"POST"}); const qualified=result.qualification_status==="business_qualified"; const partial=result.chat&&!qualified; resultBox.className=`probe-result ${qualified ? "success" : partial ? "partial" : "error"}`; const diagnostic={route_endpoint_not_found:"端点路径不匹配，请核对 base URL 与协议；不能据此认定模型不存在",credentials_rejected:"密钥被当前端点拒绝",rate_limited:"平台限流，请稍后自动重试",timeout:"连接超时",connection_failed:"连接失败"}[result.diagnostic_code]||""; const qualification=qualified?`业务已验证 ${result.verified_output_characters||0} 字符`:(result.qualification_status==="protocol_only"?"仅协议兼容，业务调用将使用安全降级":"未取得业务资格"); resultBox.textContent=`连接 ${result.chat ? "✓" : "×"} · 结构化 ${structuredCapabilityLabel(result.structured_output_capability)} · ${qualification} · 工具 ${result.tool_calling ? "✓" : "×"} · 强制工具 ${result.forced_tool ? "✓" : "×"}${diagnostic ? ` · ${diagnostic}` : ""}${result.error ? ` · ${result.error}` : ""}`; }
