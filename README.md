...
知乎短篇终审只要 15 项评分完整，就不再要求模型重复返回旧版三项总分；Runtime 会自行计算并校验。页面会分别提示“终审模型调用失败”和“终审结果未通过系统校验”，两种情况都会保留最佳稿。

终审若把逐条问题复核写成摘要，系统只会恢复问题编号与上一轮一致的条目；未匹配的问题仍按未复核处理并限制结果。这个恢复过程完全在本地完成，不会重复调用模型。
