...
The first terminal review remains full-manuscript. A later incremental review may run only when the saved baseline manuscript hash, baseline analysis hash, exact revision-source hash, and current-analysis hash all match their corresponding manuscript text. Every selected review window must be non-empty and carry an explicit selection reason. Any mismatch or unexplained scope triggers `full_fallback` with a specific reason. Broad or structural changes, semantic changes without LTP, incomplete issue reconciliation, and uncertain NLP/window mapping force a complete review; a group made exclusively of verified mechanical repairs may use local incremental review after the full local scan. Long-manuscript comparison is hierarchical by chapter marker and stable scene, with character-level comparison limited to changed paragraphs rather than the whole novel.
