...
Promotion writes a run-scoped snapshot journal before changing repair artifacts; the hash-validated `outputs/candidate.md` quality checkpoint is its commit marker. Startup or resume restores an incomplete journal and completes a marker-backed promotion idempotently. Recovery and ordinary or revision quality promotion share one process lock, so a concurrent finalize or resume cannot restore or discard the journal of an active promotion.
