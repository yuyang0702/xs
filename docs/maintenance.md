...
Ordinary polish still runs sequentially. The full review is compacted once, then each prose window receives only issues whose evidence or meaningful terms match that window, plus at most one high-priority whole-story issue. The existing narrative ledger contributes only overlapping unresolved questions, promises, setups, and scene-state changes. After a window is accepted or preserved, Runtime derives the next prompt's handoff state from that window's actual output; the next original opening and previous prose tail remain as boundary context. This does not assign seven-step structure to small polish windows and does not create another story-state authority.
